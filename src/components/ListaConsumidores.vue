<template>
  <div>
    ...
    <table class="container table">
      ...
      <tbody>
        <tr v-for="consumidor in consumidores" :key="consumidor.id">
          <td>{{ consumidor.nome }}</td>
          <td>{{ consumidor.sobrenome }}</td>
          <td>{{ consumidor.cpf }}</td>
          <td>{{ consumidor.nomeprecomunidade }}</td>
          <td class="row justify-content-end">
            ...
            <a
              ...
              @click="IdentificarCpf(consumidor.id)"
            >
          ...
          result.data.forEach(data => {
            PreComunidades.buscarPorId(data.precomunidade).then(result => {
              ...
              this.consumidores.push({
                ...
                cpf: data.cpf,
                senha: data.senha,
                nomeprecomunidade: "Comunidade não encontrada"
              });
            });
          });
        })
